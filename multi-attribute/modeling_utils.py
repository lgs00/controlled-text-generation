...
class PreTrainedModel(nn.Module, ModuleUtilsMixin):
    ...
    def _generate_no_beam_search(
        ...
        if not(gedi_model is None):
            ...
            seq_a = (torch.ones(input_ids.shape[0])*pt_id).type_as(input_ids).view(-1,1)    # 正例的标签，主题则为true，情感则为positive
            seq_b = (torch.ones(input_ids.shape[0])*nt_id).type_as(input_ids).view(-1,1)    # 负例的标签，主题则为false，情感为negertive
            # if not(multi_code is None):
            seq_a2 = torch.LongTensor(multi_code).unsqueeze(0).to(seq_a.device)     #  主题词
            seq_a3 = torch.LongTensor(senti_code).unsqueeze(0).to(seq_a.device)  # 情感词
            seq_a = torch.cat((seq_a, seq_a2, seq_a3, input_ids), dim=1)[:,:]
            seq_b = torch.cat((seq_b, seq_a2, seq_a3, input_ids), dim=1)[:,:]
            # else:
            # seq_a = torch.cat((seq_a, seq_a3,input_ids), dim=1)[:,:]   # seq_a（正例标签，主题词，prompts，device），如果是情感，则无主题词
            # seq_b = torch.cat((seq_b, seq_a3,input_ids), dim=1)[:,:]   # seq_b（负例标签，主题词，prompts，device），如果是情感，则无主题词

            bsz = seq_a.shape[0]
            seq_batched = torch.cat((seq_a,seq_b),dim=0)# 将seq_a和seq_b进行连接
            if pad_lens is None:    # None
                gedi_pad_lens = None
            else:
        ...
        if get_ll:
            sequence_ll = 0
        word_model = word2vec.Word2Vec.load("/root/nlp/word_model/word2vec.model")
        while cur_len < max_length:
            # input_ids是单纯的prompts的encoder
            model_inputs = self.prepare_inputs_for_generation(input_ids, past=past)     # 将输入变成字典，初次为{'input':tensor([[40,599,2712,9669,11]]),'past':None}
            ...
            if not(gpt3_api_key is None):
                ...
                # model_inputs是变成字典后的输入，有input_ids,和past
                outputs = self(**model_inputs)  # len(outputs)=2
                next_token_logits = outputs[0][:, -1, :]    # len(next_token_logits)=1

            if get_ll:
                next_token_logp = torch.log_softmax(next_token_logits,-1)
            if not(gedi_model is None):
                #want to compute LM loss here so feeding inputs as labels
                if not gedi_past is None:   # 有past的时候
                    input_batched = torch.cat((model_inputs["input_ids"],model_inputs["input_ids"]),dim=0)
                    # input_batched是上个时刻的词的token（整数代号）
                ...
                # gedi_outputs[0].shape[1] 第一次是6（输入词加正负标签加主题词，即输入长度加2），之后是1
                # gedi_outputs[0].shape第一次是2*6*50257，之后是2*1*50257
                if gedi_past is None:
                    if gedi_outputs[0].shape[1]>1:
                        old_logits = torch.log_softmax(gedi_outputs[0][:, :-1, :],-1)
                        shift_logits = gedi_outputs[0][..., :-1, :].contiguous() # shift_logits.shape是2*4*50257
                        shift_labels = seq_batched[..., 1:].contiguous() # shift_labels是（主题词加prompts）*2
                        ...
                        # seq_batched.shape[0]为2
                        # logits_r变成2*4的损失
                        seq_len = logits_r.shape[1]
                        logits_r = torch.sum(logits_r,1)    # logits_r为按行求和,形成两个loss
                        ...
                        logits0 = torch.stack((logits_pos,logits_neg),1)    # 将正负loss进行拼接

                        if "logit_scale" in dir(gedi_model):
                            logits0 = gedi_model.logit_scale*logits0
                            # gedi_model.logit_scale为1.3120
                            # logits0为loss*scale后，（[[-7.1359,-7.9521]]）
                        if "bias" in dir(gedi_model):
                            logits0 = logits0 + gedi_model.bias
                        if not (class_bias==0):
                            logits0[:,0] += class_bias
                        logp_desired = torch.log_softmax(logits0,-1)[:,0]   # -0.3661做softmax归一化后，进行ln（）函数计算，取出两个数
                        logp_undesired = torch.log_softmax(logits0,-1)[:,1] # -1.1823
                ...
                seq_len= seq_len+1  # 句子长度
                gedi_logits= (torch.log_softmax(gedi_outputs[0][:, -1, :],-1)+logits_r.unsqueeze(1))    # 正负词表的概率,gedi_logits.shape为2*50257
                logits_pos,logits_neg = torch.split(gedi_logits/seq_len,input_ids.shape[0]) # logits_pos为1*50257, 正词表的概率
                logits = torch.stack((logits_pos,logits_neg),2) # 变成50257*2的向量

                # logits.shape为1*50257*2
                if "logit_scale" in dir(gedi_model):
                ...
                # next_token_logits.shape为1*50257的向量，即词表概率
                ...
                # sorted_logps为概率，从小到大
                # sorted_indices为整数标签
                peak= torch.max(next_token_logits,1).values.unsqueeze(1)
                next_token_p = torch.softmax(next_token_logits,-1)  # 归一化后的词表概率

                # next_token_logits.shape[0]=1
                for i in range(0,next_token_logits.shape[0]):
                    if True:
                        p_sorted = next_token_p[i,sorted_indices[i]]
                        # sorted_indices[i]是整数标签，维度变成1维，
                        # p_sorted.shape是50257
                        cumulative_probs = torch.cumsum(p_sorted, dim=-1)
                        # cumulative_probs为按行累计求和，计算累计概率
                        logp_desired_sorted = logp_desired_t[i,sorted_indices[i]]
                        # logp_desired_sorted为正例概率，变成一维
                        # logp_desired_sorted.shape为50257
                        # math.log(target_p)为-0.2231，ln（target_p）
                        ind_to_remove = (cumulative_probs <filter_p) & (logp_desired_sorted<(math.log(target_p)))
                        # ind_to_remove为[true，···，false···]
                        # ind_to_remove.shape为50257
                        next_token_logits[i,sorted_indices[i][ind_to_remove]]-=10000    # 减去10000
                        if ind_to_remove[-1]:
            ...
            # rep_penalty_scale为10

            # repetition penalty from CTRL paper (https://arxiv.org/abs/1909.05858)
            if repetition_penalty != 1.0:
                for i in range(batch_size):
                    # cond_len为0，表示从prompts开始进行重复惩罚
                    # input_ids为单纯prompts的token
                    prevs = input_ids[i][cond_len:].tolist()
                    # prevs为inputs降成一维的list
            ...
            # 根据主题词改变词表概率
            if do_sim:
                # word_model = models.KeyedVectors.load_word2vec_format("/root/nlp/word_model/GoogleNews-vectors-negative300.bin", binary=True)
                word_logits, word_indices = torch.sort(next_token_logits, descending=True, dim=-1)
                word_list = word_indices[:, :50].tolist()[0]
                word =  [tokenizer.decode(i).strip() for i in word_list]
                # print(word_model)
                # for i in word_list:
                #     word.append(tokenizer.decode(i).strip())
                for i in range(len(word)):
                    if word[i] not in word_model:
                        word[i] = 'unk'
                vec_logit = word_model.wv[word]
                topic_temp = tokenizer.decode(multi_code).strip()
                vec_topic = word_model.wv[topic_temp].reshape(1, -1)
                logit_sim = cosine_similarity(vec_topic, vec_logit)
                for i, index_temp in enumerate(word_list):
                    if logit_sim[0,i] < 0:
                        logit_sim[0, i] = 0
                    next_token_logits[0, index_temp] = next_token_logits[0, index_temp] + sim_weight * logit_sim[0, i]
            # do_sample默认为false
            if do_sample:
            ...
            # next_token.tolist()为下一个词的token,toilst变成列表
            if not (gedi_model is None):
            ...
            # unfinished_sentstensor([1])
            # pad_token_id0
            tokens_to_add = next_token * unfinished_sents + pad_token_id * (1 - unfinished_sents)
            # tokens_to_add为下一个词的token加device
            # get_ll为false
            if get_ll:
                sequence_ll += next_token_logp[0,next_token]
            input_ids = torch.cat([input_ids, tokens_to_add.unsqueeze(-1)], dim=-1)
            # input_ids为加上了下一个词的token
            # eos_token_ids为结束词
            for eos_token_id in eos_token_ids:
        ...
        if not(gedi_model is None):
            ...
            with open("/root/nlp/Gedi_test/model_m/model_0420_last/test.txt","a") as data_json:
                data_json.write(f'GeDi estimates the probability that it sample is desired class is: = {str(torch.exp(logp_desired[0]).item())}\n')
