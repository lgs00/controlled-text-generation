...
def main():
    ...
    parser.add_argument("--text_prompt", action="store_true", help="if use prompts in text")
    parser.add_argument("--prompts_file", type=str, help="prompts file")
    parser.add_argument("--generation_file_path", type=str, help="generation file")
    parser.add_argument("--prompt_all", type=int, default=20, help="generation file")
    ...
    p_count = 0
    with open(args.generation_file_path,"w") as file_score:
        while True:
            if user_prompt:
                while True:
            ...
            if args.text_prompt:
                ...
                if args.prompts_file.endswith('.jsonl'):
                    dataset = pd.read_json(args.prompts_file, lines=True)
                    prompts = pd.json_normalize(dataset['prompt'])['text']  # json规范化
                else:
                    test_file = open(args.prompts_file, "r")
                    prompts = test_file.read().splitlines()
                    test_file.close()
                if p_count >= min(args.prompt_all, len(prompts)):
                    break
                args.prompt = prompts[p_count]
                p_count += 1
            start_len = 0
            ...
            temp_secondary_code = " " + args.secondary_code
            temp_sentiment_code = " " + args.sentiment_code
            multi_code = tokenizer.encode(temp_secondary_code)  # 对主题进行编码
            senti_code = tokenizer.encode(temp_sentiment_code)  # 对情感进行编码
            attr_class = 1
